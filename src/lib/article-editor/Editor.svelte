...
<script>
  ...
  import {
    articleBlocksArray,
    paragraphDataIdCounter,
    articleTitleAndSubtitileObj,
  } from "$lib/store";
  ...
  function removeInnerTextAndUpdateVariables(e) {
    ...
    if (e.target.textContent === "") {
      ...
      if (parentBlockDataId != null) {
        ...
        articleArrayElement = fakeArticleBlocksArray.find(
          (articleBlock) => articleBlock.dataId === parentBlockDataId
        );
  ...
  async function saveArticle(e) {
    ...
    if (
      articleTitle != "" &&
      $articleBlocksArray.find((arrayElement) => arrayElement.text != "") !=
        undefined
    ) {
      let previewSrc = "";

      if (
        $articleBlocksArray.find(
          (arrayElement) => arrayElement.type === "figure"
        ) != undefined
      ) {
      ...
      await fetch(
        `http://localhost:9000/api/upload-article/${userArticle.type}`,
        {
          method: "POST",
          headers: {
        ...
        >
          <figure>
            <div class="image-container">
              <img
                class="article-image"
                src={$articleBlocksArray[i].src}
                alt={$articleBlocksArray[i].alt}
              />
            </div>
            <figcaption
              class="image-caption"
  ...
  {#if isDOMLoaded}
    <div class="publish-container">
      <div class="article-format">
        <button class="add-button" on:click={addNewParagraph}
          >{@html icParagraph}</button
        >
      ...
      {#if $articleTitleAndSubtitileObj.type === "edit"}
        <button
          ...
          data-type="publish-draft"
          on:click={saveArticle}>Опубликовать черновик</button
        >
        <button
          class="draft-button"
          data-type="update-draft"
          on:click={saveArticle}>Сохранить изменения</button
        >
      {:else}
        <button
          class="publish-button"
          data-type="publish-new"
          on:click={saveArticle}>Опубликовать</button
        >
        <button
          class="draft-button"
          data-type="save-draft"
          on:click={saveArticle}>Сохранить как черновик</button
        >
      {/if}
    </div>
  {:else}
